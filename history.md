### 2017-11-02 ###
gogokoala:
1.测试修正sf1 api的若干问题
2.sf1 简单检索编码
3.sf1 高级检索增加日期选择组件，引入primeng库( https://www.primefaces.org/primeng/#/calendar )

puyd:
1.创建complex-search.service.ts
2.初步实现关键字条件选择和添加

### 2017-11-03 ###
puyd:
1.实现关键字段查询

steps:
1.完善api/sf1

### 2017-11-04 ###
steps:
1.sf1简单检索继续编码，增加sf1-list-resolver.service.ts

### 2017-11-05 ###
puyd:
1.各查询字段的第一条记录指定的操作符，专用于指定整个查询字段与其他字段的组合方式（以前固定为AND组合，现在可以使用OR）
2.实现日期范围查询条件
3.移动complex-search.service.ts到sf1目录下，重命名为sf1-search.service.ts

todo:
1.日期输入框的宽度，在不同的模式下，需要进行调整。

### 2017-11-06 ###
puyd:
1.ComplexSearch查询条件全局存储
2.实现buildKeySearch和buildCodeSearch
3.实现getKeyWords
4.实现数据库选择getDBValue

steps:
1. sf1-list-resolver.service.ts编码
2. sf1-list 页面数据显示
3. 讨论检索结果及二次检索的实现方法

### 2017-11-07 ###
steps:
1. 格式化sf1-search.service
2. 修正footer显示错误
3. 各页面增加user: UserService
4. 测试已有代码(进行中)
5. TODO: 高级检索面页，各条件中首条件均未显示NOT
6. 修正server/redisstore.ts使用阿里云Redis
7. 安装部署服务器
8. 解决API跨域访问CORS问题

puyd:
1. 修改查询条件的生成逻辑，放开首字段不能为NOT的限制，但是受API限制，以NOT开头的查询条件不能被执行
2. 取消原有的不同条件组之间的AND连接，取消条件组的优先级括号，修改为每个条件均按其首个有效条件（不为空）的操作符串联

### 2017-11-08 ###
steps:
1. 安装部署服务器
2. 配置服务在线升级pm2
3. 移除iconfont,统一使用font-awesome

### 2017-11-09 ###
steps:
1.配置客户端部署

### 2017-11-10 ###
puyd:
1. 实现图文模式和列表模式的切换

steps:
1. 安装艾特云服务器
2. 云服务器需要进行域名备案，否则访问受到阿里云限制，出现异常

### 2017-11-11 ###
puyd:
1. 实现二次查询
2. todo: 二次查询无结果时，会跳转到简单查询页面

steps:
1. 解决二次查询无结果的问题，增加t=?,强制刷新

### 2017-11-12 ###
puyd:
1. 初步实现分页功能，CSS需要统一设置

steps:
1.测试二次查询功能
2. todo: 二次查询的连接条件显示存在错误，增加条件后无显示

### 2017-11-13 ###
steps:
1. 调整api端响应结构，增加response.error属性{status:'XXX', message:''}
2. 增加sf1Service.redirectUrl，实现查询异常时的跳转重定向

puyd:
1. Exp实现Encode和Decode方法
2. 简化表达式的显示方式

### 2017-11-14 ###
steps:
1. 阿里云安装MSSQL2012
2. 安装Ater服务端。数据库待同步。
3. 部署客户端网站
4. 激活polyfills，使网站兼容IE11, 360浏览器
5. 调整高级检索css

puyd:
1. 修改二次检索表示式显示，可以删除二次检索的条件

### 2017-11-15 ###
steps:
1. 调整CSS布局，以兼容多种浏览器
2. 检索异常处理
3. 安装Alter服务器，恢复数据库 

### 2017-11-16 ###
puyd:
1. 调整Exp.Encoe方法，去掉常量内容

### 2017-11-17 ###
puyd:
1. 调整并完善分页功能

### 2017-11-18 ###
### 2017-11-19 ###
### 2017-11-20 ###
steps:
1. 服务端调整session实现，由客户端记录cookie:x-session-id
2. 服务端jwt调整
3. 服务端用户登录模块调整
4. 服务端sf1,ft1扩展开始

puyd:
1. 初步实现列表页面左侧的过滤统计
2. 显示查询失败时的错误信息
3. 实现高级检索页面的命中条数查询功能
todo: 测试中发现对“优先权日”字段的查询没有结果，不知道是API的问题，还是我们测试账号的限制，因此，对授权日的统计，全部为0

### 2017-11-21 ###
steps: 
1. 国知局并发有限制
2. 实现服务端访问并发限制

### 2017-11-22 ###
puyd:
1. 重写SearchExp类
2. 修改高级检索页面

### 2017-11-23 ###
puyd:
1. 修改列表页面，调整二次查询的输入方式，重新实现查询条件的删除操作

steps:
1. 实现ft1的PDF文档下载

### 2017-11-23 ###
steps:
1. 修改patent类，使用之可应用于数据库保存

### 2017-11-24 ###
steps:
1. 修改sf1,sf2，检索结果可以保存至数据库表patent
2. 调整sf1-list的显示效果

puyd:
1. 可使用过滤条件进行二次查询
2. 查询表达式中可以删除查询条件

### 2017-11-25 ###
### 2017-11-26 ###
### 2017-11-27 ###
steps:
1. 修正priority字段数据带逗号导致错误的BUG

### 2017-11-28 ###
steps:
1. 文本关键字高亮显示编码
2. sf1 list路由处理

### 2018-04-14 ###
puyd:
1. 系统授权首先区分功能模块。模块下可定义授权项目。

2. 授权项目分成四类：
2.1 登陆可见——会员登录后即可使用，比如会员资料菜单组下的各项功能
2.2 服务授权——用户需要授权之后才可以使用，比如专利检索等
2.3 管理功能——仅后台管理用户可以使用，比如文章发布等功能
2.4 内部功能——系统需要的内部功能设定，菜单上不可见

3. 授权项目的计分方式分成三类：
3.1 不计费
3.2 按期计费
3.3 按次计费

4. 用户角色分类
4.1 一般用户，按服务类功能授权使用，且按期或按次扣费
4.2 内部用户，按服务类功能授权使用，忽略项目的计费方式，不计费
4.3 内部管理，按管理类功能授权使用
4.4 系统管理，可以使用所有管理类功能

### 2018-04-18 ###
1. 权限: 包含一系列功能组,每个功能组必须要有名称和图例,并包含一系列功能列表.
    [
        {
            group:"系统维护",                                //组名称
            img:"url('assets/images/desk/cell06.jpg')",     //图例
            func:[                                          //功能列表
                {
                    id: ?,                                  //类型待定
                    label: "用户管理",                        //名称
                    href: "#/sysmain/accountmng",           //页面链接
                    kind: 0,                                //功能分类,'0-登录可见','1-服务授权','2-系统管理','3-内部功能'
                    pay: 0,                                 //计费方式,'0-不计费','1-按期计费','2-按次计费'
                },
                ....
            ]
        },
        ....
    ]
    
    权限表结构采用扁平设计：
    [
        {
            group: ?                                        //=0，表示该项为模块组。>0, 表示所属模块组的id
            id: ?,                                          //唯一标识
            label: "用户管理",                               //名称
            img:"url('assets/images/desk/cell06.jpg')",     //图例
            href: "#/sysmain/accountmng",                   //页面链接
            kind: 0,                                        //功能分类,'0-登录可见','1-服务授权','2-系统管理','3-内部功能'
            pay: 0,                                         //计费方式,'0-不计费','1-按期计费','2-按次计费'   
        }
    ]

    
2. 角色: 一个角色必须有名称和类型,自定义的角色包含一组相关功能的列表,列表中的功能可用于给相关用户授权,不在列表中的功能则禁止授权与使用。暂不使用
    [
        {
            id:0,                  //id
            label:'一般用户',       //角色名称
            func:[]                //预定义角色的功能列表为空,按系统规则处理
        },
        {   id:1,
            label:'内部用户',
            func:[]
        },
        {
            id:2,
            label:'网站维护',
            func:[]
        },
        {
            id:3,
            label:'系统管理',
            func:[]
        },
        {
            id:101,                  //自定义角色
            label:'自定义角色',
            func:[ ?,?,?,... ]       //功能列表为相关功能的id数组
        },
        ......
    ]

3. 用户: 一个用户必须有名称,并至少对应一个角色,默认角色为一般用户,并包含一系列功能授权
    [
        {
            id: 101,                    //id
            label: "puyd"               //名称
            pwd: "123456"               //登录密码
            code: "13511650262"         //手机号
            mail: "admin@aiptan.com"    //邮件地址
            role: [ ]                   //角色,id数组，保留，暂不使用
            func: [                     //功能授权
                {
                    id: 1,              //授权功能的id
                    active: true,       //是否启用
                    count:  99999,      //剩余点数，仅针对记次的功能项目
                    stop: "2099-12-31"  //终止日期，仅针对按期计费的功能项目
                },
                ...
            ]
        },
        ......
    ]

4. 功能分类kind：
  =0: 所有登录用户均可使用, 不会出现在用户授权列表中，比如"会员资料"。
  =1: 比如"专利检索", 所有的用户均可见到此项，但是需要授权以后才可以使用。
  =2：比如"文章上传"，只有授权用户才可以看到此项，并使用。
  =4: 比如"下载"，非菜单项，授权以后才可以使用。
